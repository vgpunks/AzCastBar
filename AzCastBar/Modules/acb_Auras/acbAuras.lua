...
function AuraPluginMixin:QueryAuras(unit,auraType,showSelfAuras,showPetAuras,showStealable)
	...
	while (true) do
		local name, icon, count, debuffType, duration, endTime, casterUnit, isStealable = UnitAura(unit,index,auraType);	-- [18.07.19] 8.0/BfA: "dropped second parameter"
		if (not name) then
			break;
		elseif (not isFiltered) or (showStealable and isStealable) or (showSelfAuras and casterUnit == "player") or (showPetAuras and (casterUnit == "pet" or casterUnit == "vehicle")) then
			local t = self:NewTimer(auraType,name,icon,duration,endTime);
			t.index = index;
			t.count = count;
