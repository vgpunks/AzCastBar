...
local extraOptions = {
	{
		...
		{ type = "Slider", var = "minShownCooldown", default = 10, label = "Minimum Shown Cooldown", min = 2, max = 600, step = 1, y = 12 },
...
function plugin:QueryCooldowns()
	...
	for tab = 1, MAX_SKILLLINE_TABS do
		local name, texture, offset, numSpells, bool = GetSpellTabInfo(tab);
		if (not name) then
			break;
		end
		for i = 1, numSpells do
			local start, duration, enabled = GetSpellCooldown(offset + i,BOOKTYPE_SPELL);
			if (enabled ~= 0) and (duration) and (duration > self.cfg.minShownCooldown) and (self.cfg.maxShownCooldown == 0 or duration < self.cfg.maxShownCooldown) then
				...
				local name, _, texture = GetSpellInfo(offset + i,BOOKTYPE_SPELL);
