...
-- StatusBarSetValue -- Midnight: getmetatable approach may fail; use a pcall guard
local StatusBarSetValue;
do
	local ok, val = pcall(function()
		return getmetatable(GameTooltipStatusBar).__index.SetValue;
	end);
	if ok and type(val) == "function" then
		StatusBarSetValue = val;
	else
		StatusBarSetValue = function(self, value)
			local mt = getmetatable(self);
			local fn = mt and mt.__index and mt.__index.SetValue;
...
function acb:CreateMainBar(frameType,token,options,multi)
	local f = self:CreateBar(frameType,nil,"AzCastBarPlugin"..token, BackdropTemplateMixin and "BackdropTemplate");
...
function acb:CreateBar(frameType,mainBar,name)
	local f = CreateFrame(frameType or "Frame",name,UIParent, BackdropTemplateMixin and "BackdropTemplate");
	f:SetSize(250,18);
	f:Hide();
...
SlashCmdList[modName] = function(cmd)
	...
	-- Options
	if (param1 == "") then
		local loaded, reason = C_AddOns.LoadAddOn("AzCastBarOptions");
		if (loaded) then
	...
	-- Invalid or No Command
		...
		AzMsg(format("----- |2%s|r |1%s|r ----- |1%.2f |2kb|r -----",modName,C_AddOns.GetAddOnMetadata(modName,"Version"),C_AddOns.GetAddOnMemoryUsage(modName)));
